...
// ── Promos temporales ────────────────────────────────────────────────────────
const PROMO_EXPIRY = new Date('2026-06-16T02:00:00.000Z') // 15/06 23:00 ART
...
// Calcula descuento de promo por item (proporcional al precio del item)
function calcularDescuentoPromoItem(item, items) {
  ...
  if (LLAVEROS_PROMO.has(sku)) {
    ...
    const precioPromo = _precioLlaveroPromo(cantLlaveros)
    return (item.precioUnitario - precioPromo) * item.cantidad
  }
...
export async function registrarVenta({ items, metodoPago, descCarrito = 0, empleado = '', notas = '', descuentoImanes = 0, descuentoPromo = 0 }) {
  ...
  const rows = items.map((item, idx) => {
    ...
    // Descuento de promo para este item
    const dtoPromoItem = calcularDescuentoPromoItem(item, items)

    // S = (precioTotal - descuentoImanes - descuentoPromo) - descuento%
